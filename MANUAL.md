...
```javascript
// tilePane のデフォルト z=200 より大きい値を割り当てる
...
当初は CartoDB 暗色地図を使用し、その後 `blank` タイル → `pale` タイルへと変更してきた。
現在は **国土地理院淡色地図**（`pale`）に `grayscale(100%) brightness(X)` フィルターを適用して陸地/海の色を調整している。

**陸地・海の色分離の仕組み**:
- `pale` タイルの陸地ピクセルはほぼ白（grayscale 後 ≈ 255）、海ピクセルは薄い青（grayscale 後 ≈ 209）
- `brightness(X)` でスケールダウンすることで海を暗く、陸地を薄い灰色にする
- `.leaflet-container { background: #333333 }` はタイル読み込み前のローディング時の海色として機能
...
var BASE_OPACITIES = [0.8, 0.9, 1.0];   // 地図暗 / 地図中 / 地図明
var BASE_LABELS    = ['地図暗', '地図中', '地図明'];
var baseOpacityIdx = 1;  // デフォルト: 地図中
...
| 設定 | 陸地の明度 | 海の明度 |
|------|-----------|---------|
| 地図明（brightness 0.65） | 約 #A9A9A9 | 約 #888 |
| 地図中（brightness 0.55） | 約 #909090 | 約 #737373 |
| 地図暗（brightness 0.45） | 約 #747474 | 約 #5E5E5E |

**参照**: JMA公式キキクルビューア（`/web/webapp/kikikuruViewer`）は `pale` + `grayscale(100%)` を標準設定として採用している。
...
国が指定した約 340 本の指定河川洪水予報河川を、常時水色の細い線で表示し、
洪水危険度が 2 以上（黄色以上）の河川を太い色付き線と水系名称ラベルで上書きする。

#### API

| API | URL | 説明 |
|-----|-----|------|
| 静的全河川 | `bosai/jmatile/data/map/none/none/none/surf/designated_river/data.geojson?id=map_designated_river` | 全 340 河川（時刻なし） |
| 動的危険度 | `bosai/jmatile/data/risk/{ymdhms}/none/{ymdhms}/surf/designated_river/data.geojson?id=designated_river` | level≥2 の河川のみ（時刻あり） |

動的 API には `level≥2` の河川しか含まれないため、全河川を常時表示するには**静的・動的の完全分離**が必要。

#### 層構造

| 層 | ペイン | zIndex | 内容 |
|----|--------|--------|------|
| 静的外縁（黒）| designatedRiverPane | 300 | 全河川・黒縁取り（常時） |
| 静的内色（水色） | designatedRiverPane | 300 | 全河川・水色（常時） |
| 動的外縁（黒）| designatedRiverPane | 300 | level≥2 河川・黒縁取り（危険度色） |
| 動的内色（危険度色）| designatedRiverPane | 300 | level≥2 河川・危険度色 |
| 水系名称ラベル | designatedRiverLabelPane | 600 | level≥2 河川のラベル（ズーム8以上） |

ラベルペインを zIndex:600 に配置することで河川線より前面に表示する。
...
// インデックス = ズームレベル（0〜18）
// 外縁 = drW()、内色 = Math.max(1, drW()-2)
```

#### 水系名称ラベルの実装

- ズーム 8 以上のみ表示
- `properties.name`（水系名）を `L.divIcon` で配置
- 8方向のテキストシャドウ（`text-shadow: 1px 1px 0 #000, ...`）で黒輪郭
- 文字色は危険度色と同色

```javascript
var icon = L.divIcon({
  html: '<div style="color:' + color + ';font-size:12px;font-weight:bold;' +
        'white-space:nowrap;text-shadow:1px 1px 0 #000,-1px -1px 0 #000,' +
        '1px -1px 0 #000,-1px 1px 0 #000,0 1px 0 #000,0 -1px 0 #000,' +
        '1px 0 0 #000,-1px 0 0 #000">' + name + '</div>',
  className:'', iconAnchor:[0,0]
});
L.marker([labelLat,labelLon],{icon:icon, pane:'designatedRiverLabelPane'}).addTo(map);
```
...
起動
  ↓ fetch(静的全河川 GeoJSON)
  → designatedRiverStaticData を保持
  → initDesignatedRiverStatic()：静的2層（黒縁+水色）を描画（洪水ON時のみ）

showFrame(idx) 呼び出し
  → scheduleDesignatedRiverToFrame(ymdhms)（400ms デバウンス）
  → applyDesignatedRiverToYmdhms(ymdhms)
    ↓ キャッシュあり → drawDesignatedRiverDynamic(cachedData)
    ↓ キャッシュなし → fetch(動的 GeoJSON)
      ↓ 200 OK → キャッシュ保存 → drawDesignatedRiverDynamic()
      ↓ 404    → キャッシュに null 保存（再リクエスト抑制）
```

#### 洪水レイヤー ON/OFF との連動

- 洪水ボタン OFF 時 → `clearDesignatedRiverLayers()`（静的・動的・ラベルすべて除去）
- 洪水ボタン ON 時 → `initDesignatedRiverStatic()` + 現フレームの動的データ再適用

...
1つのTypeScriptコードベースからiPhone・Android・Webブラウザの3プラットフォームに同時対応する。

```
ソースコード（TypeScript + JSX）
  ├─ Expo Build（iOS）  → ネイティブアプリ（.ipa）→ App Store
  ├─ Expo Build（Android）→ ネイティブアプリ（.apk）→ Google Play
  └─ Expo Export（Web） → 静的HTML/CSS/JS → GitHub Pages
```

レーダー・キキクル画面は Leaflet（ブラウザ専用ライブラリ）を使うため、HTML全体をテンプレートリテラルに格納し、iPhone版では `react-native-webview`（WebView）、Web版では `<iframe srcDoc>` で表示する。プラットフォーム分岐は `.native.tsx` 拡張子で expo-router が自動判別する。

`react-native-web` が React Native コンポーネントを HTML/CSS に変換するため、`View→div`・`Text→span`・`AsyncStorage→localStorage` など1つのコードで両方動作する。
...
| 日付 | 対象 | 内容 |
|------|------|------|
| 2026-06-28 | レーダー | iOS 全ブラウザでのブラックアウト・クラッシュ対策を実施。海岸線GeoJSONのbase64埋め込み（660KB）をGitHub Rawからのfetchによる動的取得に変更し、WKWebViewのメモリ超過を解消。また2026-06-26に追加したアメダス地点クリックで気象庁観測ページを開く機能を取り消し、安定版に巻き戻し |
| 2026-06-26 | キキクル | 指定河川洪水予報を常時水色表示＋危険度色上書き、水系名ラベル追加。ループ時に最新フレームで1秒停止。地図タイルをpale（国土地理院淡色地図）に変更 |
| 2026-06-26 | レーダー | アメダスマーカーに地点名ホバー表示・クリックで気象庁観測ページを開く機能を追加（Web版 iframe の `allow-popups` 対応） |
| 2026-06-17 | キキクル | 大雨・土砂・浸水レイヤーをCanvas GridLayer方式に変更（`mix-blend-mode: multiply` で白背景透過・高解像度対応） |
| 2026-06-16 | 共通 | コントロールボタンを横一列レイアウト（`flex-direction:row`）に変更。スマホでキキクルがクラッシュする問題を修正（srcDoc 470KB→70KB削減・タイルオンデマンド管理） |
| 2026-06-15 | キキクル | 洪水キキクルの奇数ズーム消え・他クアドラント混入をcanvas GridLayer方式で根本修正。アメダスオーバーレイを大幅拡張（12種対応・デフォルトON・ズーム間引き・時刻同期） |
| 2026-06-15 | 共通 | タブアイコン刷新（天気→☀️雲・キキクル→川の流れ図PNG）。webタブバー高さ修正 |
| 2026-06-14 | 全体 | キキクルビューアタブを新規追加（大雨/土砂/浸水/洪水2層構造・過去データ再生）。レーダーフレームprobe導入（古いフレームの404除外）。git push後の自動Webデプロイフックを追加 |
| 2026-05-19 | レーダー・天気 | アニメーション時間拡張（1/2/3時間）・降水強度凡例追加・過去5時間再生・お気に入り地点登録・サブ地域切替・全47都道府県58地点対応。MANUAL.md 新規作成 |
| 2026-04-19 | レーダー | レーダー+衛星タブ追加。iPhone/Web でプラットフォーム別ファイルに分離（`.native.tsx`） |
| 2026-04-18 | 全体 | 天気予報アプリ初版リリース（React Native + Expo + JMA API）。GitHub Pages デプロイ設定 |
